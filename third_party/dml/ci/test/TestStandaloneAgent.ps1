...
(
    ...
    [Parameter(Mandatory=$true)][string[]]$BuildArtifactsBranch
)
...
$UriParameters = @(
    ...
    "branchName=refs/heads/$BuildArtifactsBranch",
...
if (!$Build)
{
    Write-Warning "Could not find last successful build of $BuildArtifactsBranch branch."
    exit 1
}
